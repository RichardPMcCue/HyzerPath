	...
	const tabs = [
		{
			href: '/',
			label: 'Courses',
			match: (p: string) => p === '/' || p.startsWith('/courses'),
		...
		{
			// Route is /range, not /measure — ad blockers (EasyPrivacy list)
			// block document requests to paths ending in /measure
			href: '/range',
			label: 'Measure',
			match: (p: string) => p.startsWith('/range'),
...
<nav
	class="fixed inset-x-0 bottom-0 z-40 border-t border-edge bg-card/95 backdrop-blur"
